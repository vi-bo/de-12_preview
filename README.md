...
В лабе 1. занимаемся настройкой кластера из двух нод, настраиваем необходимые доступы, используем базовые практики devops. В супрачивке поднимаем локально kafka на первой ноде.
...
В лабе 2 и 3 делаем lambda-архитектуру с двумя слоями (batch-слоем и speed-слоем). В лабе 2 соответственно делаем batch-слой, организовываем пакетную обработку данных в хранилище. Получаем данные из kafka, используя airflow считаем агрегаты и перекладываем данные внутри clickhouse.
...
В лабе 2 и 3 делаем lambda-архитектуру с двумя слоями (batch-слоем и speed-слоем). В лабе 3 соответственно делаем speed-слой, обработку данных "на лету". Делаем real-time пайплайн Kafka --> Spark Streaming --> Kafka, получаем события из топика, считаем метрики, отправляем их в топик. 
...
В суперачивке лабы 3 работаем с основными инструментами hadoop, загружаем данные, обрабатываем их и построим датамарт.
...
* построить витрину в соответсивии с заданным условием.
...
В лабе 2 и 3 мы сделали lambda-архитектуру с двумя слоями (batch-слоем и speed-слоем). В лабе 4 делаем "сервис-слой", пишем Flask-приложение, которое возвращает данные из clickhouse по запросу, упаковываем его в Docker. 
...
В супачивке лабы 4. добавляем дашборды в сервис. Делаем две системы для дашбордов - одну с бизнес-метриками, вторую для операционных задач.
...
В лабе 5 рассчитываем витрину для маркетинговой аналитики по результатам прошедшей распродажи e-commerce портала. Осуществляем сбор данных из нескольких источников, обогащение данных маппингом товар-категория из Postgres DB, расчет почасовых агрегатов по категориям и пользователям. Сохраняем результаты в Redis.
